...
import { API_BASE_URL, API_ENDPOINTS, BLOCKCHAIN_EXPLORER_TX } from "../../lib/config";
...
export default function ForensicAdminDashboard() {
  ...
  const fetchRecords = async () => {
    try {
      const res = await fetchWithAuth(`${API_ENDPOINTS.RECORDS}?role=admin`);
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => router.push("/")}>
            ...
            <span className="font-black text-xl tracking-tight text-white">
              VERI-CHAIN <span className="text-blue-500 font-medium">ADMIN AUDIT</span>
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative">
              <div className="text-xs font-bold text-red-400 uppercase tracking-widest mb-2">Forgeries Caught</div>
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative">
              <div className="text-xs font-bold text-green-400 uppercase tracking-widest mb-2">Authentic Anchors</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 overflow-hidden flex flex-col h-[750px]">
              <div className="p-6 border-b border-white/10 flex items-center justify-between">
                <span className="font-bold text-white">Recent Audit Scans</span>
                ...
                {records.map((r) => (
                  <li
                    ...
                    className={`p-5 cursor-pointer transition-all border-l-4 ${
                      selectedRecord?.id === r.id ? "bg-blue-600/10 border-blue-500" : "border-transparent hover:bg-white/5"
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-center mt-3">
                      ...
                      <div className="text-xs text-blue-400 font-bold">{(r.fraud_score * 100).toFixed(1)}% Risk</div>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="lg:col-span-2">
              {selectedRecord ? (
                <div className="bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 p-8 h-[750px] flex flex-col overflow-y-auto">
                  <div className="flex items-center justify-between mb-6 pb-6 border-b border-white/10">
                    <div>
                      <h2 className="text-2xl font-black text-white">{selectedRecord.filename}</h2>
                      <div className="text-sm text-gray-500 mt-1 font-mono">
                        ID: {selectedRecord.id} | {selectedRecord.created_at ? new Date(selectedRecord.created_at).toLocaleString() : ""}
                      </div>
                    </div>
                    <div
                      className={`px-5 py-2 rounded-xl border font-bold text-sm shadow-xl flex items-center gap-2 ${
                  ...
                  <div className="grid grid-cols-2 gap-6 min-h-[260px] mb-6">
                    <div className="flex flex-col h-full">
                      ...
                      <div className="flex-1 bg-black/40 rounded-2xl border border-white/5 overflow-hidden flex items-center justify-center p-2 relative group">
                        <img
                          src={`${API_BASE_URL}/api/v1/uploads/${selectedRecord.original_file_path.split(/[\\/]/).pop()}`}
                          alt="Original"
                    ...
                    <div className="flex flex-col h-full">
                      <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2 justify-between">
                        ...
                        <span className="px-2 py-0.5 bg-red-500/20 text-red-400 rounded text-[9px] font-bold tracking-wider">
                          TAMPER MAP
                        </span>
                      </div>
                      <div className="flex-1 bg-black rounded-2xl border border-red-500/20 overflow-hidden flex items-center justify-center p-2 relative group">
                        <img
                          src={`${API_BASE_URL}/api/v1/uploads/${selectedRecord.ela_file_path.split(/[\\/]/).pop()}`}
                          alt="ELA Heatmap"
                          className="w-full h-full object-contain"
                        />
                      </div>
                  ...
                  <div className="bg-black/30 p-6 rounded-2xl border border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-y-4 text-sm">
                      ...
                      <div className="font-mono text-blue-400 break-all bg-blue-500/10 px-2 py-1 rounded inline-block">
                        {selectedRecord.file_sha256}
                      </div>

                      <div className="text-gray-500">Polygon Amoy Tx</div>
                      {selectedRecord.tx_hash ? (
                        <a
                          href={BLOCKCHAIN_EXPLORER_TX(selectedRecord.tx_hash)}
                          target="_blank"
                          rel="noreferrer"
                          className="font-mono text-blue-400 break-all hover:underline bg-blue-500/10 px-2 py-1 rounded inline-block"
                        >
                          {selectedRecord.tx_hash}
                        </a>
                      ) : (
                        <div className="font-mono text-gray-500 bg-white/5 px-2 py-1 rounded inline-block">
                          None (Rejected)
                        </div>
                      )}

                      <div className="text-gray-500">Gemini Verdict</div>
                      <div className="capitalize text-white bg-white/5 px-2 py-1 rounded inline-block w-fit">
                        {selectedRecord.gemini_verdict?.replace(/_/g, " ")}
                      </div>

                      <div className="text-gray-500">Forensic Risk Score</div>
                      <div className="text-white bg-white/5 px-2 py-1 rounded inline-block w-fit font-bold">
                        {(selectedRecord.fraud_score * 100).toFixed(1)}% Fraud Probability
                      </div>
                    </div>

                    {selectedRecord.forensic_analysis && (
